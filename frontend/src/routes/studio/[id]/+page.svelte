<script lang="ts">
	...
	import { studiosApi, coverApi } from '$lib/api';
	...
	async function load() {
		loading = true;
		studio = await studiosApi.get(id);
		thumbUrls = {};
		titleCovers = {};
		if (studio) {
			if (studio.mainImage) {
				mainImgUrl = await coverApi.dataURL(studio.mainImage);
			}
			for (const f of studio.images || []) {
				await ensureThumb(f);
			}
			for (const t of studio.titles || []) {
	...
	function lbStep(dir: number) {
		if (!studio || studio.images.length === 0) return;
		lightboxIdx = (lightboxIdx + dir + studio.images.length) % studio.images.length;
	}
	...
	async function deleteStudio() {
		if (!studio) return;
		if (!confirm('Удалить эту студию?')) return;
	...
	function titlesLabel(n: number): string {
		...
		const m100 = n % 100;
		let word = 'тайтлов';
		if (m10 === 1 && m100 !== 11) word = 'тайтл';
		else if (m10 >= 2 && m10 <= 4 && (m100 < 12 || m100 > 14)) word = 'тайтла';
		return `Выпустила ${n} ${word}`;
	}
</script>
...
{:else}
	<div class="page">
		<div class="back">
			...
			<button class="btn" onclick={() => goto(`/studio/${id}/edit`)}>
				<Edit size={14} />
				Редактировать
			</button>
			<button class="btn btn-danger" onclick={deleteStudio}><Trash2 size={14} /> Удалить</button>
		</div>
		...
		<div class="detail-header">
			<div class="poster">
				...
				{:else}
					<div class="poster-placeholder">{displayName(studio.names)[0]}</div>
				{/if}
			</div>
			<div class="header-info">
				<div class="type-badge">Студия</div>
				<h1 class="detail-title">{studio.names.map((n) => n.value).join(' / ')}</h1>
				{#if studio.founded}
					<div class="info-line">Дата создания: {studio.founded}</div>
				{/if}
				{#if studio.founders.length > 0}
					<div class="info-line">Основатели: {studio.founders.join(', ')}</div>
				{/if}
				{#if studio.titles.length > 0}
					<div class="in-titles">{titlesLabel(studio.titles.length)}</div>
				{/if}
			</div>
		</div>
		...
		{#if lightboxIdx >= 0 && studio}
			<div class="lightbox">
				<button class="lb-backdrop" aria-label="Закрыть" onclick={() => (lightboxIdx = -1)}></button>
				<img class="lb-img" src={thumbUrls[studio.images[lightboxIdx]]} alt="" />
				{#if studio.images.length > 1}
					...
					<span class="lb-counter">{lightboxIdx + 1} / {studio.images.length}</span>
				{/if}
