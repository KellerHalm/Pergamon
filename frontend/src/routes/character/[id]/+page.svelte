<script lang="ts">
	...
	async function load() {
		loading = true;
		character = await charactersApi.get(id);
		thumbUrls = {};
		titleCovers = {};
		if (character) {
			if (character.mainImage) {
				mainImgUrl = await coverApi.dataURL(character.mainImage);
			}
			for (const f of character.images || []) {
				await ensureThumb(f);
			}
	...
	function lbStep(dir: number) {
		...
		lightboxIdx = (lightboxIdx + dir + character.images.length) % character.images.length;
	}
	...
	function inTitlesLabel(n: number): string {
		const m10 = n % 10;
		const m100 = n % 100;
...
{:else}
	<div class="page">
		<div class="back">
			...
			<button class="btn" onclick={() => goto(`/character/${id}/edit`)}>
				<Edit size={14} />
				Редактировать
			</button>
			<button class="btn btn-danger" onclick={deleteCharacter}><Trash2 size={14} /> Удалить</button>
		</div>
		...
		<div class="detail-header">
			<div class="poster">
				...
				{:else}
					<div class="poster-placeholder">{displayName(character.names)[0]}</div>
				{/if}
			</div>
			<div class="header-info">
				<div class="type-badge">Персонаж</div>
				<h1 class="detail-title">{character.names.map((n) => n.value).join(' / ')}</h1>
				{#if character.age}
		...
		{#if character.titles.length > 0}
			<section class="section">
				<h3>Тайтлы</h3>
				<div class="cards">
					{#each character.titles as t (t.id)}
		...
		{#if lightboxIdx >= 0 && character}
			<div class="lightbox">
				<button class="lb-backdrop" aria-label="Закрыть" onclick={() => (lightboxIdx = -1)}></button>
				<img class="lb-img" src={thumbUrls[character.images[lightboxIdx]]} alt="" />
				{#if character.images.length > 1}
					...
					<span class="lb-counter">{lightboxIdx + 1} / {character.images.length}</span>
				{/if}
