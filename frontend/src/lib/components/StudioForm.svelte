<script lang="ts">
	...
	let { studio = null }: { studio?: Studio | null } = $props();
	...
	onMount(async () => {
		...
		if (studio) {
			names = studio.names.length > 0 ? studio.names.map((n) => ({ ...n })) : [{ kind: 'original', value: '' }];
	...
	async function save() {
		...
		try {
			const st: Studio = {
				...
				founders: founders.map((f) => f.trim()).filter(Boolean),
				description,
				images: [...images],
				titles: [],
				titleIds: [...titleIds],
...
<div class="form">
	...
	<div class="field">
		...
		<input class="input" placeholder="Например: 1986 или 1986-12-24…" bind:value={founded} />
	</div>

	<div class="field">
		<span class="label">Основатели</span>
		{#each founders as f, i}
			<div class="row-2">
				<input class="input" placeholder="Имя основателя…" bind:value={founders[i]} />
				<button class="btn btn-icon sm" onclick={() => founders.splice(i, 1)}><Minus size={14} /></button>
			</div>
		{/each}
		<button class="btn sm" onclick={addFounder}><Plus size={14} /> Добавить основателя</button>
	</div>

	<div class="field">
		<span class="label">Описание</span>
		<textarea class="textarea" rows={4} bind:value={description} placeholder="История студии…"></textarea>
	</div>
	...
	<div class="field">
		...
		{#if titleIds.length > 0}
			...
			{/each}
		{:else}
			<span class="hint">Студия пока не привязана ни к одному тайтлу.</span>
		{/if}
