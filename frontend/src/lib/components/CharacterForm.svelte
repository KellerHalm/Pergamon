<script lang="ts">
	...
	import { charactersApi, coverApi, titlesApi, errText } from '$lib/api';
	...
	let { character = null }: { character?: Character | null } = $props();
	...
	onMount(async () => {
		...
		if (character) {
			names = character.names.length > 0 ? character.names.map((n) => ({ ...n })) : [{ kind: 'original', value: '' }];
	...
	async function save() {
		...
		try {
			...
			const id = await charactersApi.save(c);
			goto('/character/' + id);
		} catch (e: any) {
...
<div class="form">
	...
	<div class="field">
		...
		{#if titleIds.length > 0}
			...
			{/each}
		{:else}
			<span class="hint">Персонаж пока не привязан ни к одному тайтлу.</span>
		{/if}
